...
{
    public partial class frm02RegistroPreContratos : XtraForm
    {
        List<EContrato> lstContrato = new List<EContrato>();
        bool nuevo = false;
        public frm02RegistroPreContratos()
        {
            InitializeComponent();
        }
        ...
        private void modificartoolStripMenuItem5_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                frm.lkpFomaMante.Enabled = false;
            }
            catch (Exception ex)
            {
        ...
        private void controlDeReprogramacionesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Obe == null)
                return;
            frmManteRegistroReprogramaciones frm = new frmManteRegistroReprogramaciones();
            frm.MiEvento += new frmManteRegistroReprogramaciones.DelegadoMensaje(reload);
            frm.ObeC = Obe;
            frm.Show();
            frm.setValues();
            frm.Text = string.Format("Contrato N° {0} - Reprogramaciones", Obe.cntc_vnumero_contrato);
        ...
        private void pagoFomaYFinanciamientoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            EContrato Obe = (EContrato)viewContrato.GetRow(viewContrato.FocusedRowHandle);
            if (Obe == null)
                return;
            frmListarFomaFinanciamiento frm = new frmListarFomaFinanciamiento();
            frm.Obe = Obe;
            frm.cntc_icod_contrato = Obe.cntc_icod_contrato;
            frm.Text = $"Foma/ Financiamiento Contrato N°{Obe.cntc_vnumero_contrato}";
            frm.Show();

        }

        private void registroDeTipoPlanYPlanToolStripMenuItem_Click(object sender, EventArgs e)
        {
            EContrato Obe = (EContrato)viewContrato.GetRow(viewContrato.FocusedRowHandle);
            if (Obe == null)
                return;
            FrmRegistroTipoSepulturaPlan frm = new FrmRegistroTipoSepulturaPlan();
            frm.Text = $"Contrato {Obe.cntc_vnumero_contrato}";
            frm.Obe = Obe;
            frm.cargar();
        ...
        private void registroDeFomaPorNivelToolStripMenuItem_Click(object sender, EventArgs e)
        {
            EContrato Obe = (EContrato)viewContrato.GetRow(viewContrato.FocusedRowHandle);
            if (Obe == null)
                return;
